...
Additionally, the speed of the transformation can be defined as:

\begin{equation} \label{eq:velocity}
    \vec{v} = \pderiv{\x}{t} = \pderiv{\vec{\phi} (\X, \, t)}{t}
\end{equation}

Based on the velocity, it is possible to define the material velocity gradient as:

\begin{equation} \label{eq:material_velocity_gradient}
    \Fdot = \tensorTwoGradO{\vec{v}} = \pderiv{\vec{v}}{\X}
\end{equation}

The spatial velocity gradient can also be defined as:

\begin{equation} \label{eq:spatial_velocity_gradient}
    \lll = \tensorTwoGrad{\vec{v}} = \pderiv{\vec{v}}{\x} = \dot{\F} \F^{-1} 
\end{equation}

The deformation rate tensor can be defined as the symmetric part of the spatial velocity gradient:

\begin{equation} \label{eq:spatial_deformation_rate}
    \ddd = \frac{1}{2} \left( \lll + \lll^T \right)
\end{equation}

It can also be related to the material deformation rate tensor $\Edot$ as follows:

\begin{equation} \label{eq:material_deformation_rate}
    \Edot = \F^T \ddd \F
\end{equation}

The material deformation rate tensor can also be written as:

\begin{equation} \label{eq:material_deformation_rate_2}
    \Edot = \frac{1}{2} \Cdot = \frac{1}{2} \left( \dot{\F}^T \F + \F^T \dot{\F} \right)
\end{equation}
...
\begin{equation}
    \potential (\C, \, \X) = \int \frac{1}{2}\pkTwo : \dot{\C} \dt 
\end{equation}
